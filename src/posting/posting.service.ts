...
export class PostingService {
  ...
  async createPosting(
    ...
    // 발행 시 발행 일시 설정
    const publishedAt =
      postingData.status === PostingStatus.PUBLISHED ? new Date() : null;
  ...
  async updatePosting(
    ...
    if (
      postingData.status === PostingStatus.PUBLISHED &&
      !existingPosting.published_at
    ) {
    ...
    if (
      postingData.status === PostingStatus.ARCHIVED &&
      !existingPosting.archived_at
    ) {
    ...
    if (hasCreatorOnlyFields) {
      ...
      if (!creator) {
        throw new ForbiddenException(
          "크리에이터 전용 기능입니다. 크리에이터로 전환 후 이용해주세요."
        );
      }
    }
  }
